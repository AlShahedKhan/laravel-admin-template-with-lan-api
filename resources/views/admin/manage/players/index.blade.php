...
@section('admin_content')
    <div class="page-content">
        ...
        <div class="page-header">
            <div class="row">
                <div class="col-sm-6">
                    <title>{{ ___('cricket.players') }}</title>
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ ___('cricket.home') }}</a></li>
                        <li class="breadcrumb-item">{{ ___('cricket.players') }}</li>
                    </ol>
                </div>
            </div>
        </div>
        ...
        <div class="table-content table-basic mt-20">
            <div class="card">
                <div class="card-header">
                    <h4 class="mb-0">{{ ___('cricket.players') }}</h4>
                    @if (hasPermission('player_create'))
                        ...
                        <table class="table table-bordered role-table" id="example1">
                            <thead class="thead">
                                <tr>
                                    ...
                                    <th class="purchase">{{ ___('cricket.player_slug') }}</th>
                                    <th class="purchase">{{ ___('cricket.player_runs') }}</th>
                                    <th class="purchase">{{ ___('cricket.player_wickets') }}</th>
                                    <th class="purchase">{{ ___('cricket.man_women') }}</th>
                                    <th class="purchase">{{ ___('cricket.team_name') }}</th>
                                    @if (hasPermission('player_update') || hasPermission('player_delete'))
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('player.store') }}" method="Post">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="from-group">
                            <label for="player_runs">{{ ___('cricket.player_runs') }}</label>
                            <input type="text" class="form-control" name="player_runs" required="">
                        </div>
                        <div class="from-group">
                            <label for="player_wickets">{{ ___('cricket.player_wickets') }}</label>
                            <input type="text" class="form-control" name="player_wickets" required="">
                        </div>
                        <div class="from-group">
    ...
    <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">{{ ___('cricket.update_player') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true" class="close_button">&times;</span>
                    </button>
                </div>
