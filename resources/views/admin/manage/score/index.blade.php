...
@section('admin_content')
    <div class="page-content">
        ...
        <div class="page-header">
            <div class="row">
                <div class="col-sm-6">
                    <title>{{ ___('cricket.batter_first') }}</title>
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{ ___('cricket.home') }}</a></li>
                        <li class="breadcrumb-item">{{ ___('cricket.batter_first') }}</li>
                    </ol>
                </div>
            </div>
        </div>
        ...
        <div class="table-content table-basic mt-20">
            <div class="card">
                <div class="card-header">
                    <h4 class="mb-0">{{ ___('cricket.batter_first') }}</h4>
                    @if (hasPermission('score_create'))
                        ...
                        <table class="table table-bordered role-table" id="example1">
                            <thead class="thead">
                                <tr>
                                    ...
                                    @if (hasPermission('score_update') || hasPermission('score_delete'))
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($BatterRuns > 0 and $BatterRuns == 0)
                                                {{ number_format($BatterRuns * 100, 2, '.', '') }}
                                            @elseif ($Balls > 0 and $BatterRuns == 0)
                                                {{ number_format($Balls * $BatterRuns, 2, '.', '') }}
                                            @elseif ($Balls == 0 and $BatterRuns == 0)
                                                {{ number_format($Balls * $BatterRuns, 2, '.', '') }}
                                            @elseif ($Balls == 0 and $BatterRuns > 0)
                                                {{ number_format($BatterRuns * 100, 2, '.', '') }}
                                            @elseif ($BatterRuns > 0 and $Balls >= 0)
                                                {{ number_format(($BatterRuns / $Balls) * 100, 2, '.', '') }}
                                            @endif
                                        </td>
                                        @if (hasPermission('score_update') || hasPermission('score_delete'))
                                            <td>
                                                @if (hasPermission('score_update'))
    ...
    <div class="modal fade" id="teamModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('score.store') }}" method="Post" class="score_add_form">
                    @csrf
                    <div class="row mb-3">
                        <div class="col-md-12">
                            <div class="row mb-3">
