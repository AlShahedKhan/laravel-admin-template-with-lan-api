...
@section('title', 'Cricket Commentary - Commentary')
...
@section('admin_content')
    ...
    <h4 class="fw-bold py-3 mb-4">
        <span><a href="{{ route('cricket') }}">{{ ___('cricket.cricket') }}</a> /</span>
        {{ ___('cricket.stats') }}
    </h4>
    ...
    <div class="row">
        <div class="col-md-12">
            <ul class="nav nav-pills flex-column flex-md-row mb-3">
                <li class="nav-item">
                    <a class="nav-link{{ request()->routeIs('cricketMatchScoreStatus') ? ' active' : '' }}"
                        href="{{ route('cricketMatchScoreStatus', ['matchId' => $managePublicTable[0]->match_id]) }}">
                        <i class="bx bx-user me-1"></i>
                        {{ ___('cricket.scoreboard') }}
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link{{ request()->routeIs('cricketMatchCommentryStatus') ? ' active' : '' }}"
                        href="{{ route('cricketMatchCommentryStatus', ['matchId' => $managePublicTable[0]->match_id]) }}">
                        <i class="bx bx-user me-1"></i>
                        {{ ___('cricket.commentary') }}
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link{{ request()->routeIs('cricketMatchStatsStatus') ? ' active' : '' }}"
                        href="{{ route('cricketMatchStatsStatus', ['matchId' => $managePublicTable[0]->match_id]) }}">
                        <i class="bx bx-user me-1"></i>
                        {{ ___('cricket.stats') }}
                    </a>
                </li>
                                    ...
                                    <tbody class="tbody">
                                        ...
                                        @foreach ($batterfirsts as $key => $row)
                                            @php
                                                ...
                                                $out_type = $row->scoreupdate->out_type;
                                                $out_by_type = $row->scoreupdateoutbytype->out_by_type;
                                                $one = $row->scoreupdateone->one ?? 0;
                                                $two = $row->scoreupdatetwo->two ?? 0;
                                                $three = $row->scoreupdatethree->three ?? 0;
                                                $four = $row->scoreupdatefour->four ?? 0;
                                                $six = $row->scoreupdatesix->six ?? 0;
                                                $Balls = $row->scoreupdateballsplayed->balls_played ?? 0;
                                                $BatterRuns = $one + $two * 2 + $three * 3 + $four * 4 + $six * 6;
                                            @endphp
                                            <tr>
                                                <td>{{ ___('cricket.' . $player_name) }}</td>
                                                <td>{{ ___('cricket.' . $out_type) }}</td>
